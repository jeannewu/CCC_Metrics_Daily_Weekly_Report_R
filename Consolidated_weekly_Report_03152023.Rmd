...
This report contains data as of `r format(Sys.Date()-6, '%d %B, %Y')`. Atypical records under investigation are not included in the metrics tables and may result in Total columns that do not add up to 100%
...
breaks0 <- seq(100,100*round(max_y0/100,0),100)
##Amelia's comments: 
...
Fig4.timetrend <- ggplot(recr_veri_act_wk,aes(y=Verified_Activities_n1,x=as.Date(recruit.week.date),color=verified.activities)) + 
  geom_line() +  geom_point() + scale_color_manual(values = c("blue","purple","lightblue"), 
...
break2 <- seq(2,10*round(max(recr_veri_act$total_wdconsent/10),0),2)
...
ggplot(aes(y=value,x=as.Date(recruit.week.date),color=type)) + 
  geom_line() +  geom_point() + scale_color_manual(values = c("red","orange","green"), 
                     name="Refusal Withdrawal vs Invitations" ,labels=c("# request data destroyed","# Withdraw consent","# withdrawal per 1K invitations"))+
  scale_y_continuous(name="# of Participants Refusal \n/ Requested Data Destroyed", limits=c(0,10*round(max(recr_veri_act$total_wdconsent/10),0)),breaks=break2,sec.axis = sec_axis( ~ . /2  , name = "# Invitations",breaks=break2/2)) +
...
recrui_wk_verified1.site <- recrui_wk_verified1.site %>% 
  ...
  for (i in 1:9){
    ...
    break_y0 <- seq(break_int0,10*round(max_y0/10,1),break_int0)
  ...
  coord_cartesian(ylim =c(0,max_y0)) +
      scale_x_date(name="Date",limit=as.Date(c("2021-12-31",currentDate)),date_labels = "%y-%m-%d", date_breaks = "2 weekly",expand=c(0,0))  +
...
ggplot(aes(y=value.new,x=as.Date(recruit.week.date),color=type)) + 
  ...
  coord_cartesian(ylim =c(0,max_ys/a)) +
  scale_x_date(name="Date",limit=as.Date(c("2021-12-31",currentDate)),date_labels = "%y-%m-%d", date_breaks = "2 weeks",expand=c(0,0))  +
  ggtitle(paste("Weekly Counts of Withdrawals and Rates by Invitations \nthrough", currentDate,s,sep=" "))+
  theme( axis.text.x=element_text(angle=60, hjust=1), axis.title.x = element_text( size = 14, face = "bold") ,
         axis.title.y = element_text(size = 14, face = "bold",color="black"),
...
```{r, eval=TRUE,echo=FALSE,fig.cap='Figure 5'}
knitr::opts_chunk$set(warning = FALSE, message = FALSE) 
print(plot.data)
```


```{r, eval=TRUE,echo=FALSE,fig.cap='Figure 5.combo',results='asis'}
  for (i in 1:9){
    s <- levels(veri_resp$site)[i]
...
```{r, eval=TRUE,echo=FALSE,fig.cap='Figure 6'}
knitr::opts_chunk$set(warning = FALSE, message = FALSE) 
fig4
```
...
write.csv(ca_hist[,c("date","token","Connect_ID","site", "age","sex", "race","Verification status","Recruitment type",
                     "User Profile submitted","RcrtSI_TypeTime_v1r0","RcrtUP_SubmitTime_v1r0","Cancer Type",
                     "Cancer Comments","Previous Cancer","Cancer Year","RcrtV_CStatusMatch_v1r0")], paste(outputpath,"prod_previous_cancer_cases_profile_complete_",currentDate,".csv",sep=""), row.names = F,na="")

box_setwd(dir_id=191948877738)
box_write(object = ca_hist[,c("date","token","Connect_ID","site", "age","sex", "race","Verification status","Recruitment type",
                              "User Profile submitted","RcrtSI_TypeTime_v1r0","RcrtUP_SubmitTime_v1r0","Cancer Type",
                              "Cancer Comments","Previous Cancer","Cancer Year","RcrtV_CStatusMatch_v1r0")], 
                              paste("prod_previous_cancer_cases_profile_complete_",currentDate,".csv",sep=""),row.names = F,na="")
...
for (i in 1:length(levels(refwd.cases.notes$site))) {
  ...
  write.csv(refwd.cases.notes[which(refwd.cases.notes$site == levels(refwd.cases.notes$site)[i]),
                              which(!grepl("Date",colnames(refwd.cases.notes)))],paste(outputpath,"RefusalWithdrawl_cases_",site,currentDate,".csv",sep=""),row.names = F,na="")
}
...
file_name =paste("Incentives_Weekly_Report_UChicago",currentDate,".csv",sep=""))

write.csv(incentive[,c("token","site","NORC.Payment.Eligibility","Eligible.for.Payment","Time.Payment.Eligible.for.Round")],
          paste(outputpath,"Incentives_Weekly_Report_UChicago_",currentDate,".csv", sep=""),na="",row.names=F)
