...
The working data applied is the recruitment data updated from 'nih-nci-dceg-connect-prod-6d04.FlatConnect.participants_JP' synchronouslly updated with the ongoing study, without any inactive participants, or duplicates found in the active recruitments.
...
#
 #recr.vars <- c("d_512820379","d_821247024","d_230663853","d_919254129","d_699625233","state_d_684926335","state_d_849518448", "state_d_119643471","state_d_706256705", "state_d_435027713",state_d_934298480,d_827220437,d_914594314,d_471593703,
 #             state_d_158291096,d_311580100,d_914639140,d_878865966,d_167958071, state_d_684635302, 
...
bq <- bq_project_query(project, query="SELECT Connect_ID, token, d_512820379,d_821247024,d_230663853,d_919254129,d_699625233,state_d_684926335,state_d_849518448,state_d_119643471,state_d_706256705, state_d_435027713,state_d_934298480,d_827220437,d_914594314,d_471593703,
              state_d_158291096,d_311580100,d_914639140,d_878865966,d_167958071, d_684635302, 
...
#saveRDS(recr_noinact_wl1,"~/Documents/CONNECT_projects/Biospecimen_Feb2022/Jing_projects/biospecQC_03082022/data/recruit.rds")
...
#a. overall verified status by site
  ...
  recr_step$total <- recr_step$step_0 +recr_step$step_1
...
# data$recrt_steps_v1r1 <-factor(data$recrt_steps_v1r0,levels=c("Never Signed In","Signed in No Consent","Consented No Profile", "Profile Verification Incomplete","Verification Complete"),exclude=NULL)

    
    work_flow_a <- data  %>% select(site, recrt_steps_v1r0) %>% tbl_cross(
       row = site,
      ...
      bold_labels() %>%
      italicize_levels() %>% 
      modify_header(label ~ "Work Flow") %>%
      modify_caption("Table 5. Current Status in Workflow Total") %>%
      as_kable()
    
    work_flow_act <- data[which(data$d_512820379 == 486306141),] %>% select(site, recrt_steps_v1r0) %>% tbl_cross(
      row = site,
      col = recrt_steps_v1r0,
      percent = "column",
      digits=c(0,2)) %>%
    ...
    work_flow_pass <- data[which(data$d_512820379 == 854703046),] %>% select(site, recrt_steps_v1r0) %>% tbl_cross(
      row = site,
      col = recrt_steps_v1r0,
      percent = "column",
      digits=c(0,2)) %>%
...
##the Table 8 profile complete
 data$verified <- factor(data$verified,exclude=NULL, levels=c("Not yet verified","Verified","Cannot be verified","Duplicate","Outreach timed out"))
  ...
  italicize_levels() %>% 
  modify_header() %>%
  modify_caption("Table 8a.Active Recruits Verifaction Status with profile completion") %>%
  as_kable()
...
#  699625233	Autogenerated flag when User Profile submitted	User Profile submitted	RcrtUP_Submitted_v1r0  
    ...
    Table8a_opt_act <- data %>% filter(d_512820379==486306141) %>% select(site, Preconsent) %>% tbl_cross(
      ...
      bold_labels() %>%
      italicize_levels() %>% 
      modify_header(label ~ "Pre-Consent Opt-Out") %>%
...
#Table 9.  Timing of Pre-consent Opt Out Among Active Recruits**") %>%	  
    ...
    active$site <- gsub("\\.","\\ ",rownames(active))
   table_precst_act <- merge(active,precst_act_w, by ="site",all.x=TRUE)

   
   table_precst_act$`Never Signed In` <-ifelse(table_precst_act$site != "X  Total" & is.na(table_precst_act$`Never Signed In`), 0,
                                           ifelse(table_precst_act$site !="X  Total" & table_precst_act$`Never Signed In` >0,table_precst_act$`Never Signed In`,
                                           sum(precst_act_w$`Never Signed In`) ))
   
     
   table_precst_act$`Signed In, no Consent` <-ifelse(table_precst_act$site != "X  Total" & is.na(table_precst_act$`Signed In, no Consent`), 0,
                                               ifelse(table_precst_act$site !="X  Total" & table_precst_act$`Signed In, no Consent` >0,table_precst_act$`Signed In, no Consent`,
                                                      sum(!is.na(precst_act_w$`Signed In, no Consent`))))  
...
table_precst_act$site <- ifelse(table_precst_act$site=="X  Total", "Total", table_precst_act$site)
 ...
 Table9_precst_act <- table_precst_act[c(2:10,1),c(1,7,8,2)]
 ...
 #Table 10.  Verification Status Among Those That Have Submitted User Profile
    Table10_profile <- data[which(data$recrt_steps_v1r0 %in% c("Profile Verification Incomplete ","Signed in, No Consent ","Verification complete")),] %>%
      select(site, verified) %>% tbl_cross(
        row = site,
        col = verified,
        percent = "row",
        digits = c(0,2),
        missing = "always",
        missing_text = "Outreach timed out",
        margin_text = "Total Submitted User Profile") %>%
      bold_labels() %>%
      italicize_levels() %>% 
      modify_header(label ~ "Verification Status") %>%
      modify_caption("Table 11.  Timing of Pre-consent Opt Out Among Active Recruits") %>%
      as_kable() 
...
#Table 13. Sex of Verified Participants
  data$sex1 <-factor(data$sex,levels=c("Female","Male", "Intersex or Other","Unknown"), order=TRUE)
    
   table13 <- data[which(data$d_821247024 == 197316935),] %>% select(site, sex) %>% tbl_cross(
      row = site,
      col = sex,
      percent = "row",
      digits=c(0,2)) %>%
...
#Table 14. Completion of Initial Survey of Verified Participants
   ...
   table14 <- data[which(data$d_821247024 == 197316935),] %>% select(site, basesvcomplt) %>% tbl_cross(
     row = site,
     col = basesvcomplt,
     percent = "row",
     digits=c(0,2)) %>%
     bold_labels() %>%
     italicize_levels() %>% 
     #modify_header(label ~ "Baseline Initial Survey Completion") %>%
     modify_caption("Table 15. Completion of Initial Survey of Verified Participants") %>%
     as_kable()
...
#Table 18. Sex of All Records
   table18_sexall <- data[which(data$d_827220437>0),] %>% select(site, sex) %>% tbl_cross(
     row = site,
     col = sex,
     percent = "row",
     digits=c(0,2)) %>%
...
#Table 19. All Records by Site
   ...
   table19_recrall <- data[which(data$d_827220437>0),] %>%
     ...
     bold_labels() %>%
     italicize_levels() %>% 
     modify_header(label ~ "Recruitment type") %>%
...
# the total response rate: where %=(N Total Verified/N Total recruits)*100   
  ###the total active recruits: active recruits:
   ...
   verified <- dcast(verfied_types, site ~ reruit_type, value.var = "count")
    ...
    cumu_response.rate$active.response.rate <- ifelse(cumu_response.rate$site ==" Total",round(100*sum(verified$Active)/cumu_response.rate$total,2), 
                                                      round(100*cumu_response.rate$Active/cumu_response.rate$total,2))
    ...
    cumu_response.rate$Active.Response <- ifelse(cumu_response.rate$site !=" Total", paste0(cumu_response.rate$Active, " (",cumu_response.rate$active.response.rate, "%)"), paste0(sum(verified$Active), " (",cumu_response.rate$active.response.rate,"%)"))
    
    cumu_response.rate$Passive.Response <- ifelse(cumu_response.rate$site !=" Total", paste0(cumu_response.rate$Passtive, " (",cumu_response.rate$passive.response.rate,"%)"), paste0(sum(verified$Passtive), " (",cumu_response.rate$passive.response.rate, "%)"))
    
    cumu_response.rate$Total.Response <- ifelse(cumu_response.rate$site !=" Total", paste0(cumu_response.rate$Passtive + cumu_response.rate$Active, " (",cumu_response.rate$passive.response.rate + cumu_response.rate$active.response.rate,"%)"), paste0(sum(verified$Passtive+verified$Active), " (",cumu_response.rate$passive.response.rate + cumu_response.rate$active.response.rate,"%)"))
 ...
 #Table 21.Completed Baseline Survey and Sample among Verified Participants and Response Ratio
  ...
  table21_surveycomplt <- data %>% filter(d_821247024 == 197316935) %>% 
   dplyr::select(site, survey_complete) %>% tbl_cross(
    ...
    bold_labels() %>%
    italicize_levels() %>% 
    modify_header(label ~ "completion of baseline survey and Sample Collections") %>%
    modify_caption("Table 21. Completed Baseline Survey and Sample among Verified Participants and Completion Ratio") %>%
    as_kable()  
...
#Table 22. Baseline Biospecimen Collected among Verified Participants
    table22_biocol <-  data %>% filter(d_821247024 == 197316935) %>% 
      ...
      bold_labels() %>%
      italicize_levels() %>% 
      modify_header(label ~ "Sample Collections") %>%
      modify_caption("Table 22. Baseline Biospecimen Collected among Verified Participants") %>%
      as_kable() 
...
Fig1 <-  ggplot(recr_count1[which(recr_count1$stage_new >0 ),], aes(x = stage, y = count,fill=fct_rev(site))) + 
  ...
  ggtitle("Cumulative Status of Recruitment (Active and Passive)") + 
...
Fig2 <- ggplot(UP_active_verified, aes(x = verified, y = count,fill=fct_rev(site))) + 
  ...
  ggtitle("Active Recruits Verification Status among those with Profile Completed") + 
  theme(panel.background = element_blank(),
...
Fig3 <- ggplot(UP_passive_verified, aes(x = verified, y = count,fill=fct_rev(site))) + 
  ...
  stat_summary(fun.y = sum, aes(label = ..y.., group = verified), geom = "text",vjust = -.2) +
  ggtitle("Passive Recruits Verification Status among those with Profile Completed") + 
  theme(panel.background = element_blank(),
        legend.title = element_text(size=7),
        axis.line = element_line(size=0.2),
...
Fig4 <-  ggplot(Age_verified, aes(x = age, y = count,fill= fct_rev(site))) + 
  ...
  ggtitle("Age of Verified Participants") + 
  theme(panel.background = element_blank(),
        legend.title = element_text(size=7),
        axis.line = element_line(size=0.2),
        axis.text.x = element_text(hjust = 0.5,size = 8, face = "bold"),                                                            
...
#c("#D53E4F","#F46D43","#FDAE61","#FEE08B","#E6F598", "#ABDDA4", "#66C2A5", "#3288BD", "#5E4FA2")
  ...
  Fig5 <- ggplot(sex, mapping=aes(x = sex, y = count,fill= fct_rev(site))) + 
    ...
    ggtitle("Sex of Verified Participants") + 
    theme(panel.background = element_blank(),
          legend.title = element_text(size=7),
          axis.line = element_line(size=0.2),
          axis.text.x = element_text(hjust = 0.5,size = 8, face = "bold"),                                                            
 ...
 #Fig6. "Race of Verified Participants"
  race <- data[ which(data$d_821247024==197316935),] %>% 
   ...
   Fig6 <- ggplot(race, mapping=aes(x = race, y = count,fill= fct_rev(site))) + 
    ...
    ggtitle("Fig6. Race of Verified Participants") + 
    theme(panel.background = element_blank(),
 ...
 #Fig7. "Completion of Initial Survey among Verified Participants"
  ...
  Fig7 <- ggplot(Survey, mapping=aes(x = basesvcomplt, y = count,fill= fct_rev(site))) + 
    ...
    #coord_cartesian(expand = FALSE)+
    #scale_y_continuous(expand = c(0.1, 0.1), limits = c(-1, NA))  + 
    ggtitle("Fig7. Completion of Initial Survey among Verified Participants") + 
    theme(panel.background = element_blank(),
          legend.title = element_text(size=7),
          axis.line = element_line(size=0.2),
          axis.text.x = element_text(hjust = 0.5,size = 8, face = "bold"),                                                       
...
#Fig8. "Completion of SSN among Verified Participants as of 05172022"
  ...
  Fig8 <- ggplot(SSN, mapping=aes(x = SSN, y = count,fill= fct_rev(site))) + 
    geom_bar(position="stack",stat="identity",na.rm=TRUE)+ 
    scale_colour_manual(values= c("#D53E4F","#F46D43","#FDAE61","#FEE08B","#E6F598", "#ABDDA4", "#66C2A5", "#3288BD", "#5E4FA2"))+
    scale_fill_manual(values= c("#D53E4F","#F46D43","#FDAE61","#FEE08B","#E6F598", "#ABDDA4", "#66C2A5", "#3288BD", "#5E4FA2"))+
    ...
    #coord_cartesian(expand = FALSE)+
    #scale_y_continuous(expand = c(0.1, 0.1), limits = c(-1, NA))  + 
    ggtitle("Fig8. Completion of SSN among Verified Participants") + 
    theme(panel.background = element_blank(),
          legend.title = element_text(size=7),
          axis.line = element_line(size=0.2),
          axis.text.x = element_text(hjust = 0.5,size = 8, face = "bold"),                                                            
...
#tables
```````{r,eval=TRUE,echo = FALSE}
knitr::kable(recruittypetotal[,c(4,1:3)], caption = 'Table 1.  the total number of Active and Passive recruits', row.names=FALSE, align=c("l", "c", "c", "c"))

knitr::kable(cumulative_all[,c(5,1:4)], caption = 'Table 2. Cumulative Status Total', row.names=FALSE, align=c("l", "c", "c", "c","c", "c")) %>% kable_styling(latex_options = "scale_down")


knitr::kable(cumulative_active[,c(1,8:13)], caption = 'Table 3. Cumulative Status for Active Only', row.names=FALSE, align=c("l", "c", "c", "c", "c","c")) %>% kable_styling(latex_options = "scale_down")

knitr::kable(cumulative_passive[,c(1,8:13)], caption = 'Table 4. Cumulative Status for Passive Only',  row.names=FALSE, align=c("l","c","c","c","c","l")) %>% kable_styling(latex_options = "scale_down")
